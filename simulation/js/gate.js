    ...
    switch (this.type) {
      case "AND":
        if (this.inputs.length >= 2) {
          ...
          console.warn("AND gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "OR":
        if (this.inputs.length >= 2) {
          ...
          console.warn("OR gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "NOT":
        if (this.inputs.length >= 1) {
          ...
          console.warn("NOT gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "NAND":
        if (this.inputs.length >= 2) {
          ...
          console.warn("NAND gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "NOR":
        if (this.inputs.length >= 2) {
          ...
          console.warn("NOR gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "XOR":
        if (this.inputs.length >= 2) {
          ...
          console.warn("XOR gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "XNOR":
        if (this.inputs.length >= 2) {
          ...
          console.warn("XNOR gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "THREEIPNAND":
        if (this.inputs.length >= 3) {
          this.output = !(
            getOutput(this.inputs[0]) &&
            getOutput(this.inputs[1]) &&
            getOutput(this.inputs[2])
          );
        } else {
          console.warn("3-NAND gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
      case "Output":
        if (this.inputs.length >= 1) {
          ...
          console.warn("Output gate does not have enough inputs:", this.id);
          this.output = null;
        }
        break;
    }
...
function getOutput(input) {
  if (!input || !Array.isArray(input) || input.length < 2) {
    console.warn("getOutput called with invalid input:", input);
    return null;
  }
  ...
  if (!gate) {
    console.warn("getOutput called with null/undefined gate");
    return null;
  }
...
function addGate(event) {
  ...
  if (type === "3-NAND") type = "THREEIPNAND";
...
export function checkConnections() {
  for (let gateId in gates) {
    ...
    if (gate.inputPoints.length != gate.inputs.length) {
      ...
      gate.type !== "Clock" &&
      (!gate.isConnected || gate.outputs.length == 0) &&
      !gate.isOutput
    ) {
      printErrors("Highlighted component not connected properly\n", id);
      return false;
  ...
  for (let gateId in gates) {
    ...
    if (gate.isOutput) {
      if (!gate.inputs || gate.inputs.length === 0) {
        console.warn("Output gate has no inputs:", gate.id);
        continue;
      }
      let input = gate.inputs[0];
      let element = document.getElementById(gate.id);
      if (!element) {
        console.warn("Element not found for gate:", gate.id);
        continue;
      }
      if (getOutput(input)) {
...
export function submitCircuit() {
  ...
  if (window.currentTab === "task1") {
    ...
    if (!checkConnectionsRS() || !checkConnections()) return;
    ...
    if (!checkConnectionsJK() || !checkConnections()) return;
